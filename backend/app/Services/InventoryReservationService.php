...
class InventoryReservationService
{
    ...
    public function reserve(
        ...
        return $this->withLockedTenant($tenant, true, fn (Tenant $lockedTenant): array => $lockedTenant->run(
            fn (): array => DB::transaction(fn (): array => $this->reserveWithinTransaction(
                $lockedTenant, $referenceType, $referenceId, $lines, $ttlSeconds,
                $idempotencyKey, $actorType, $actorUserId, $source, $requestId, $payload,
            )),
        ));
    }

    /**
     * Transaction-aware order integration. The caller must hold the central tenant lock,
     * initialize the exact tenant and own the surrounding tenant transaction.
     *
     * @param  list<array{productId: string, quantity: int}>  $lines
    ...
    public function reserveWithinTransaction(
        ...
        if ($ttlSeconds < 60 || $ttlSeconds > 1800 || $lines === []
            || count(array_unique(array_column($lines, 'productId'))) !== count($lines)) {
            throw new InventoryConflict('The reservation contract is invalid.', 'inventory_reservation_lines_invalid');
        }
        foreach ($lines as $line) {
            if ((int) $line['quantity'] <= 0) {
                throw new InventoryConflict('Reservation quantities must be positive.', 'inventory_reservation_lines_invalid');
        ...
        return $this->operations->storeResult(
            (string) $operation->id,
            $this->reservationResult($tenant, $operation, false),
        );
    }
    ...
    private function terminal(
        ...
        return $this->withLockedTenant($tenant, $fullReadiness, fn (Tenant $lockedTenant): array => $lockedTenant->run(
            fn (): array => DB::transaction(fn (): array => $this->terminalWithinTransaction(
                $lockedTenant,
                $reservationId,
                $target,
                $operationKind,
     ...
     * @return array<string, mixed>
     */
    public function terminalWithinTransaction(
        Tenant $tenant,
        string $reservationId,
        InventoryReservationStatus $target,
        InventoryOperationKind $operationKind,
    ...
    /** @return array<string, mixed> */
    public function commitWithinTransaction(Tenant $tenant, string $reservationId, string $idempotencyKey, InventoryActorType $actorType, ?string $actorUserId, string $source, ?string $requestId = null): array
    {
        return $this->terminalWithinTransaction(
            $tenant,
            $reservationId,
            InventoryReservationStatus::Committed,
            InventoryOperationKind::ReservationCommit,
            InventoryMovementKind::Commit,
            $idempotencyKey,
            $actorType,
            $actorUserId,
            $source,
    ...
    /** @return array<string, mixed> */
    public function releaseWithinTransaction(Tenant $tenant, string $reservationId, string $idempotencyKey, InventoryActorType $actorType, ?string $actorUserId, string $source, ?string $requestId = null): array
    {
        return $this->terminalWithinTransaction(
            $tenant,
            $reservationId,
            InventoryReservationStatus::Released,
            InventoryOperationKind::ReservationRelease,
            InventoryMovementKind::Release,
            $idempotencyKey,
            $actorType,
            $actorUserId,
            $source,
    ...
    /** @return array<string, mixed> */
    public function expireWithinTransaction(Tenant $tenant, string $reservationId, ?string $requestId = null): array
    {
        return $this->terminalWithinTransaction(
            $tenant,
            $reservationId,
