  ...
  const sections: Record<StorefrontSectionId, React.ReactNode> = {
    ...
    trust: (
      <section className="rounded-3xl border p-5 shadow-sm" style={{ backgroundColor: cardBackground, borderColor }}>
        <div className="mb-4"><h2 className="text-lg font-black" style={{ color: secondaryCardAccent }}>معلومات المتجر والخدمة</h2><p className="mt-1 text-xs" style={{ color: bodyColor }}>بيانات منشورة من إعدادات المتجر الحالية.</p></div>
        {facts.length > 0 ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {facts.map(({ key, label, icon: Icon }) => <div key={key} className="flex items-center gap-3 rounded-2xl border p-4 text-xs font-bold" style={{ backgroundColor: pageBackground, borderColor, color: bodyColor }}><Icon className="h-4 w-4 shrink-0" style={{ color: primaryPageAccent }} /><span>{label}</span></div>)}
          </div>
        ) : <div className="rounded-2xl border border-dashed p-6 text-center text-sm font-bold" style={{ borderColor, color: bodyColor }}>لم يضف المتجر معلومات الخدمة بعد</div>}
      </section>
    ),
    categories: (
      <section className="space-y-4">
        ...
        {categories.length > 0 ? <div className="flex flex-wrap gap-2">{categories.map((category) => <button key={category} type="button" onClick={() => onSelectCategory(category)} className="min-h-11 rounded-full border px-4 py-2 text-xs font-black transition hover:-translate-y-0.5 motion-reduce:transform-none" style={{ backgroundColor: cardBackground, borderColor, color: secondaryCardAccent }}>{category}</button>)}</div> : <div className="rounded-2xl border border-dashed p-6 text-center text-sm font-bold" style={{ backgroundColor: cardBackground, borderColor, color: bodyColor }}>لا توجد تصنيفات منشورة بعد.</div>}
      </section>
    ),
    featured_products: (
      <section className="space-y-4">
        ...
        ) : <div className="rounded-2xl border border-dashed p-8 text-center text-sm font-bold" style={{ backgroundColor: cardBackground, borderColor, color: bodyColor }}>لم ينشر المتجر منتجات بعد.</div>}
      </section>
    ),
    about: (
