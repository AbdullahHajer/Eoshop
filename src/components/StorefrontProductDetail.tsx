...
export default function StorefrontProductDetail({ product, config, primaryColor, secondaryColor, onBack, onAdd }: Props) {
  ...
  const outOfStock = product.manageStock === true && typeof available === "number" && available <= 0;
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 text-right animate-fadeIn">
      ...
      <div className="grid gap-8 md:grid-cols-2 md:items-start">
        <div className="space-y-3">
          ...
          {images.length > 1 && <div className="flex flex-wrap gap-2">{images.map((image, index) => <button type="button" key={image} onClick={() => setImageIndex(index)} className={`h-14 w-14 overflow-hidden rounded-xl border-2 ${index === imageIndex ? "border-sky-600" : "border-slate-200"}`}><img src={image} alt="" className="h-full w-full object-cover" referrerPolicy="no-referrer" /></button>)}</div>}
        </div>
        <div className="space-y-5">
          <div><span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{product.category || "غير مصنف"}</span><h1 className="mt-3 text-2xl font-black leading-tight md:text-3xl" style={{ color: secondaryColor }}>{product.name}</h1></div>
          <p className="text-sm leading-8 text-slate-600">{product.description || "لم يضف المتجر وصفًا لهذا المنتج بعد."}</p>
          <div className="rounded-2xl border border-slate-200 bg-white p-5"><span className="text-xs text-slate-500">السعر</span><p className="mt-1 text-3xl font-black" style={{ color: primaryColor }}>{product.price} {config.currency}</p></div>
          <div className={`rounded-2xl border p-4 text-xs font-black ${outOfStock ? "border-rose-200 bg-rose-50 text-rose-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>
            {outOfStock ? "غير متوفر حاليًا" : product.manageStock === true && typeof available === "number" ? `المتاح للطلب: ${available}` : "يؤكد المتجر التوفر عند معالجة الطلب"}
          </div>
          <div className="flex items-center gap-3"><div className="flex items-center rounded-xl border border-slate-200 bg-white"><button type="button" aria-label="تقليل الكمية" onClick={() => setQuantity((value) => Math.max(1, value - 1))} className="p-3"><Minus className="h-4 w-4" /></button><span className="min-w-10 text-center text-sm font-black">{quantity}</span><button type="button" aria-label="زيادة الكمية" onClick={() => setQuantity((value) => Math.min(typeof available === "number" && product.manageStock ? Math.max(1, available) : 99, value + 1))} className="p-3"><Plus className="h-4 w-4" /></button></div><button type="button" disabled={outOfStock} onClick={() => onAdd(product, quantity)} className="flex flex-1 items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-50" style={{ backgroundColor: primaryColor }}><ShoppingBag className="h-4 w-4" />إضافة إلى السلة</button></div>
