    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 text-right animate-fadeIn">
      ...
      <div className="grid gap-8 md:grid-cols-2 md:items-start">
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-col items-stretch gap-3 min-[360px]:flex-row min-[360px]:items-center"><div className="flex items-center justify-center rounded-xl border border-slate-200 bg-white"><button type="button" aria-label="تقليل الكمية" onClick={() => setQuantity((value) => Math.max(1, value - 1))} className="min-h-11 min-w-11 p-3"><Minus className="h-4 w-4" /></button><span className="min-w-10 text-center text-sm font-black">{quantity}</span><button type="button" aria-label="زيادة الكمية" onClick={() => setQuantity((value) => Math.min(typeof available === "number" && product.manageStock ? Math.max(1, available) : 99, value + 1))} className="min-h-11 min-w-11 p-3"><Plus className="h-4 w-4" /></button></div><button type="button" disabled={outOfStock} onClick={() => onAdd(product, quantity)} className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-black disabled:cursor-not-allowed disabled:opacity-50" style={{ backgroundColor: primaryColor, color: primaryForeground }}><ShoppingBag className="h-4 w-4" />إضافة إلى السلة</button></div>
