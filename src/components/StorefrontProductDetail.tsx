...
export default function StorefrontProductDetail({ product, config, primaryColor, secondaryColor, cartQuantity = 0, onBack, onAdd }: Props) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 text-right animate-fadeIn">
      <button type="button" onClick={onBack} className="inline-flex min-h-11 items-center gap-2 rounded-xl border px-4 py-2 text-xs font-black" style={{ backgroundColor: cardBackground, borderColor, color: secondaryPageAccent }}><ArrowRight className="h-4 w-4" />العودة إلى المنتجات</button>
      <div className="grid gap-8 md:grid-cols-2 md:items-start">
        ...
        <div className="space-y-5">
          ...
          <p className="text-sm leading-8" style={{ color: bodyColor }}>{product.description || "لم يضف المتجر وصفًا لهذا المنتج بعد."}</p>
          <div className="rounded-2xl border p-5 shadow-sm" style={{ backgroundColor: cardBackground, borderColor }}><span className="text-xs" style={{ color: bodyColor }}>السعر</span><p className="mt-1 text-3xl font-black" style={{ color: primaryCardAccent }}>{product.price} {config.currency}</p><p className="mt-2 text-[11px]" style={{ color: bodyColor }}>يُثبت السعر النهائي والمخزون عند إرسال الطلب إلى الخادم.</p></div>
