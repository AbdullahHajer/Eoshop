    ...
    <footer data-storefront-footer className="border-t px-4 pb-24 pt-10 lg:pb-10" style={{ backgroundColor: cardBackground, borderColor }}>
      <div className="mx-auto grid max-w-7xl gap-8 text-right sm:grid-cols-2 lg:grid-cols-4">
        <section className="space-y-3 sm:col-span-2 lg:col-span-1">
          ...
          <p className="max-w-sm text-sm leading-7 text-slate-600">{config.slogan || "مرحبًا بكم في متجرنا."}</p>
        </section>
        ...
        <section className="space-y-3">
          ...
          {social.length > 0 ? <div className="flex flex-wrap gap-2">{social.map(({ label, href, icon: Icon }) => <a key={label} href={href!} target="_blank" rel="noreferrer" aria-label={`فتح ${label}`} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border" style={{ borderColor, color: accentColor }}><Icon className="h-4 w-4" /></a>)}</div> : <p className="text-xs text-slate-500">لا توجد حسابات اجتماعية منشورة.</p>}
        </section>
      </div>
      <div className="mx-auto mt-8 max-w-7xl border-t pt-5 text-center text-[11px] text-slate-500" style={{ borderColor }}>
        <p>© {new Date().getFullYear()} {config.storeName} — جميع الحقوق محفوظة.</p>
        {attribution && <p className="mt-1 text-[10px] text-slate-400">{attribution}</p>}
      </div>
    </footer>
  );
}
