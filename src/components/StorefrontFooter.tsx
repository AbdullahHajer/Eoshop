...
export default function StorefrontFooter({ config, primaryColor, secondaryColor, cardBackground, borderColor, attribution, onNavigate }: Props) {
  ...
  const tileClass = isElegant ? "" : "rounded-2xl border p-4";
  const tileStyle = isElegant ? undefined : { borderColor };
  const social = [
  ...
  return (
    <footer data-storefront-footer data-storefront-footer-template={isElegant ? "elegant-editorial" : "tech-bento"} className="border-t px-4 pb-24 pt-10 lg:pb-10" style={{ backgroundColor: cardBackground, borderColor }}>
      <div className={`mx-auto grid max-w-7xl text-right sm:grid-cols-2 ${isElegant ? "gap-8 lg:grid-cols-4" : "gap-3 lg:grid-cols-6"}`}>
        <section className={`space-y-3 sm:col-span-2 ${isElegant ? "lg:col-span-1" : "lg:col-span-2"} ${tileClass}`} style={tileStyle}>
        ...
        <nav aria-label="روابط تذييل المتجر" className={`space-y-3 ${tileClass}`} style={tileStyle}>
        ...
        <section className={`space-y-3 ${isElegant ? "" : "lg:col-span-2"} ${tileClass}`} style={tileStyle}>
        ...
        <section className={`space-y-3 ${tileClass}`} style={tileStyle}>
