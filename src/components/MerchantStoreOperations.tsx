...
export default function MerchantStoreOperations({
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#f5f7fb] text-slate-900">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8"><div className="flex min-w-0 items-center gap-3"><button type="button" onClick={onBack} className="rounded-xl border border-slate-200 p-2 text-slate-600" aria-label="العودة إلى متاجري"><ArrowRight className="h-4 w-4" /></button><div className="min-w-0"><p className="truncate font-black">{store.storeName}</p><p className="truncate text-[11px] text-slate-500">مركز تشغيل المتجر · {user.fullName}</p></div></div><button type="button" onClick={onLogout} className="inline-flex items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-black text-rose-700"><LogOut className="h-4 w-4" /> خروج</button></div></header>

      <div className="mx-auto grid max-w-[1500px] gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[260px_minmax(0,1fr)] lg:px-8">
        <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-24"><div className="mb-4 rounded-2xl bg-slate-950 p-4 text-white"><p className="text-xs font-bold text-sky-300">المتجر المحدد</p><p className="mt-1 truncate font-black">{store.storeName}</p><p className="mt-1 truncate text-[11px] text-slate-300">{store.requestedDomain || store.internalDomain || "العنوان قيد التجهيز"}</p></div><nav aria-label="وحدات تشغيل المتجر" className="space-y-1.5">{modules.map((module) => <button key={module.section} type="button" disabled={!module.enabled} onClick={() => navigate(module)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-right text-sm transition disabled:cursor-not-allowed disabled:opacity-40 ${section === module.section ? "bg-sky-50 font-black text-sky-800" : "font-bold text-slate-600 hover:bg-slate-50"}`}><module.icon className="h-4 w-4" /><span>{module.label}</span></button>)}</nav></aside>
