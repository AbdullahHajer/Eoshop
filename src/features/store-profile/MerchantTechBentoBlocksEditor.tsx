...
export default function MerchantTechBentoBlocksEditor({ config, activeTenantId, mediaOwnerKey, onChange, uploadAsset }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {GROUPS.map((group) => {
        const blocks = placementBlocks(config, group.placement);
        const imageLedDiscovery = group.placement === "discovery";
        return (
              ...
              <details key={block.id} className="rounded-2xl border border-slate-200 bg-white p-3" open={index === 0}>
                ...
                <div className="mt-4 space-y-4 border-t border-slate-100 pt-4">
                  ...
                  <div className={`grid gap-3 ${imageLedDiscovery ? "" : "sm:grid-cols-2"}`}>
                    ...
                    {!imageLedDiscovery ? <label className="space-y-1"><span className="text-[11px] font-bold text-slate-600">نص الزر</span><input value={block.ctaLabel} maxLength={40} onChange={(event) => patchBlock(group.placement, block.id, { ctaLabel: event.target.value })} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs" /></label> : null}
                  </div>
                  {!imageLedDiscovery ? <label className="block space-y-1"><span className="text-[11px] font-bold text-slate-600">الوصف المساند</span><textarea value={block.subtitle ?? ""} maxLength={180} rows={2} onChange={(event) => patchBlock(group.placement, block.id, { subtitle: event.target.value || undefined })} className="w-full resize-none rounded-xl border border-slate-200 px-3 py-2 text-xs" /></label> : null}
                  ...
                  {!imageLedDiscovery ? <BlockAppearanceEditor block={block} fallbackSurface={group.fallbackSurfaces[index % group.fallbackSurfaces.length]} onPatch={(patch) => patchBlock(group.placement, block.id, patch)} /> : null}
                  <div className={`grid gap-3 ${imageLedDiscovery ? "sm:grid-cols-2" : "sm:grid-cols-3"}`}>
                    {!imageLedDiscovery ? <label className="space-y-1"><span className="text-[11px] font-bold text-slate-600">التعتيم: {block.overlayOpacity ?? 44}%</span><input type="range" min="0" max="100" value={block.overlayOpacity ?? 44} onChange={(event) => patchBlock(group.placement, block.id, { overlayOpacity: Number(event.target.value) })} className="w-full" /></label> : null}
