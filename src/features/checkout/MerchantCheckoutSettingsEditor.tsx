...
function Toggle({ checked, label, onChange }: { checked: boolean; label: string; onChange: (value: boolean) => void }) {
  return <button type="button" role="switch" aria-label={label} aria-checked={checked} onClick={() => onChange(!checked)} className={`flex w-full items-center justify-between rounded-xl border px-3 py-2.5 text-xs font-black ${checked ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-slate-200 bg-slate-50 text-slate-600"}`}><span>{label}</span><span>{checked ? "مفعّل" : "متوقف"}</span></button>;
}
...
export default function MerchantCheckoutSettingsEditor({ config, onChange, onOpenPreview }: Props) {
  ...
  return <div className="space-y-5" dir="rtl">
    <header className="rounded-2xl bg-gradient-to-l from-emerald-950 to-slate-900 p-5 text-white">
      <div className="flex items-start justify-between gap-3"><div><h3 className="text-base font-black">الدفع وسياسة الطلب</h3><p className="mt-1 text-xs leading-6 text-emerald-100">اعرض فقط الوسائل التي تملك بيانات حقيقية. التسعير النهائي والتحقق ينفذهما الخادم.</p></div>{onOpenPreview && <button type="button" onClick={onOpenPreview} className="inline-flex shrink-0 items-center gap-1 rounded-xl bg-white/10 px-3 py-2 text-xs font-black"><Eye className="h-4 w-4" /> معاينة الدفع</button>}</div>
    </header>

    <section className={card}><h4 className="flex items-center gap-2 text-sm font-black"><ReceiptText className="h-4 w-4 text-emerald-600" /> نصوص صفحة الطلب</h4>
    ...
    <section className={card}><h4 className="flex items-center gap-2 text-sm font-black"><CreditCard className="h-4 w-4 text-indigo-600" /> وسائل الدفع المدعومة</h4>
      ...
      <div className="space-y-2">{wallets.map((item, index) => <div key={item.id} className="rounded-xl border border-slate-200 p-3"><div className="flex items-center gap-2"><input className={input} value={item.name} onChange={(e) => updateWallet(index, { name: e.target.value })} /><button type="button" aria-label="حذف المحفظة" onClick={() => set("customWallets", wallets.filter((_, i) => i !== index))} className="rounded-lg p-2 text-rose-600"><Trash2 className="h-4 w-4" /></button></div><div className="mt-2 grid gap-2 sm:grid-cols-2"><input dir="ltr" className={input} value={item.accountNumber} onChange={(e) => updateWallet(index, { accountNumber: e.target.value })} placeholder="رقم الحساب/المحفظة" /><input className={input} value={item.accountName ?? ""} onChange={(e) => updateWallet(index, { accountName: e.target.value })} placeholder="اسم صاحب الحساب" /></div><div className="mt-2"><Toggle checked={item.active === true} label={`تفعيل ${item.name || "المحفظة"}`} onChange={(value) => updateWallet(index, { active: value })} /></div></div>)}</div>
      <div className="grid gap-2 sm:grid-cols-3"><input className={input} aria-label="اسم المحفظة الجديدة" placeholder="اسم المحفظة" value={wallet.name} onChange={(e) => setWallet({ ...wallet, name: e.target.value })} /><input dir="ltr" className={input} aria-label="رقم حساب المحفظة الجديدة" placeholder="رقم الحساب" value={wallet.accountNumber} onChange={(e) => setWallet({ ...wallet, accountNumber: e.target.value })} /><input className={input} aria-label="اسم مستفيد المحفظة الجديدة" placeholder="اسم المستفيد" value={wallet.accountName} onChange={(e) => setWallet({ ...wallet, accountName: e.target.value })} /></div><button type="button" onClick={addWallet} className="inline-flex items-center gap-1 rounded-xl bg-slate-900 px-3 py-2 text-xs font-black text-white"><Plus className="h-4 w-4" /> إضافة محفظة</button>
    ...
    <section className={card}><h4 className="flex items-center gap-2 text-sm font-black"><BadgePercent className="h-4 w-4 text-amber-600" /> كوبونات الخصم</h4><Toggle checked={config.enableCoupons === true} label="تفعيل الكوبونات" onChange={(value) => set("enableCoupons", value)} />
      {coupons.map((item, index) => <div key={item.code} className="flex items-center gap-2 rounded-xl border border-slate-200 p-2"><span className="flex-1 font-mono text-xs font-black">{item.code}</span><input aria-label={`خصم ${item.code}`} type="number" min="0.01" max="100" step="0.01" className="w-20 rounded-lg border px-2 py-1 text-xs" value={item.discountPercent} onChange={(e) => updateCoupon(index, { discountPercent: Number(e.target.value) })} /><button type="button" onClick={() => updateCoupon(index, { active: !item.active })} className={`rounded-lg px-2 py-1 text-[10px] font-black ${item.active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{item.active ? "فعال" : "متوقف"}</button><button type="button" aria-label={`حذف ${item.code}`} onClick={() => set("customCoupons", coupons.filter((_, i) => i !== index))} className="p-1 text-rose-600"><Trash2 className="h-4 w-4" /></button></div>)}
      <div className="flex gap-2"><input className={input} placeholder="CODE10" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} /><input aria-label="نسبة الخصم الجديدة" type="number" min="0.01" max="100" step="0.01" className="w-24 rounded-xl border px-3" value={couponDiscount} onChange={(e) => setCouponDiscount(Number(e.target.value))} /><button type="button" onClick={addCoupon} className="rounded-xl bg-slate-900 px-3 text-white"><Plus className="h-4 w-4" /></button></div>
    </section>
