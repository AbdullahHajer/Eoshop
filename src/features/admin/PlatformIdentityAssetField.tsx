...
export default function PlatformIdentityAssetField({ administration, purpose, label, value, placeholder, disabled, invalid, onChange }: Props) {
  ...
  const upload = async () => {
    ...
    setError(null);
    try {
      const asset = await administration.uploadPlatformAsset(purpose, file);
  ...
  return (
    <div className="space-y-2 rounded-2xl border border-slate-200 p-3">
      <label className="block text-xs font-bold">
        ...
        <input aria-label={`رابط ${label}`} dir="ltr" className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100" value={value ?? ""} maxLength={2048} onChange={(event) => onChange(event.target.value || null)} placeholder={placeholder} />
      </label>
      ...
      {previewUrl && <img src={previewUrl} alt={`معاينة ${label} قبل الرفع`} className="h-32 w-full rounded-xl object-cover" />}
