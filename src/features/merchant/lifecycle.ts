...
export function deriveMerchantLifecycle(store: StoreSubmission): MerchantLifecycleViewModel {
  ...
  if (store.verificationStatus === "rejected") {
    return lifecycle({
      ...
      nextAction: "راجع سبب الإعادة. ستُضاف آلية التصحيح وإعادة الإرسال في WP 5.5.",
  ...
  if (store.publicationStatus === "rejected") {
    return lifecycle({
      ...
      nextAction: "ستتوفر معالجة طلب النشر وإعادة إرساله ضمن WP 5.5.",
      actionOwner: "merchant",
      completedSteps: 3,
      canOpenBuilder,
      isPublished: false,
  ...
  return lifecycle({
    ...
    explanation: "يمكنك إدارة المتجر الآن. التحكم المباشر بالنشر من حساب التاجر سيضاف في WP 5.5.",
    nextAction: "ينفذ فريق المنصة النشر حاليًا حتى تتوفر لك صلاحية النشر المباشر.",
    actionOwner: "platform",
    completedSteps: 3,
    canOpenBuilder,
    isPublished: false,
