...
4. يحمل ciphertext معرّف المفتاح `kid`. مفتاح التسويق مستقل عن `APP_KEY` ولا يملك fallback إليه: يلزم `MARKETING_LINK_TOKEN_KEY_CURRENT_ID` و`MARKETING_LINK_TOKEN_KEY_CURRENT`، وغيابهما أو فساد المادة ذات 32 byte يغلق إنشاء/قراءة الرابط بالفشل. يقبل فك التشفير current أو `MARKETING_LINK_TOKEN_KEY_PREVIOUS_ID` و`MARKETING_LINK_TOKEN_KEY_PREVIOUS` فقط. للتدوير تُنقل قيم current القديمة إلى previous، ويُنشر current جديد، ثم تعيد الصيانة التشفير تحت قفل السجل. يبقى hash والرابط ثابتين، ولا تزال إزالة previous ممنوعة حتى يثبت عدم بقاء صف يحمل old `kid`.
...
يعيد resolver التحقق من صيغة token وتطابق النطاق قبل لمس tenant storage؛ لذلك يبقى الخطأ في أي منهما 404. بعد ذلك فقط تتحول أعطال اتصال/مخطط/استعلام tenant المتوقعة إلى `/` بلا UTM أو كوبون أو attribution ودون تسجيل token.
...
4. التراجع الإنتاجي roll-forward: يمكن تعطيل endpoints/resolver مع إبقاء الجداول والبيانات. لا تُسقط الجداول التي تحتوي تاريخًا؛ `down()` يقفل الجداول الموجودة كلها بترتيب ثابت عبر PostgreSQL `ACCESS EXCLUSIVE` داخل transaction قبل فحصها وإسقاطها، ويرفض عند وجود أي سجل. لذلك يبقى التراجع مسموحًا في بيئة غير إنتاجية فقط إذا كانت الجداول فارغة ولا توجد كتابة متزامنة ملتزمة.
